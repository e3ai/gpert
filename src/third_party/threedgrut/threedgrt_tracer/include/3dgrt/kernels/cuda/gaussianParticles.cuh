...
__device__ inline bool processHit(
    ...
    if (acceptHit) {
        ...
        if (particlesSphCoefficients && radiance) {
...
__device__ inline void processHitBwd(
    ...
    if ((gres > minParticleKernelDensity) && (galpha > minParticleAlpha)) {
        ...
        float3 residualRayRad = make_float3(0.0f);
        if (particleRadiancePtr) {
            float3 sphCoefficients[SPH_MAX_NUM_COEFFS];
            fetchParticleSphCoefficients(
                particleIdx,
                particleRadiancePtr,
            ...
            if (particleRadianceGradPtr) {
                grad = radianceFromSpHBwd(sphEvalDegree, &sphCoefficients[0], rayDirection, weight, radianceGrad, (float3*)&particleRadianceGradPtr[particleIdx * SPH_MAX_NUM_COEFFS * 3]);
            } else {
                grad = radianceFromSpH(sphEvalDegree, &sphCoefficients[0], rayDirection);
            }

            // >>> rayRadiance = accumulatedRayRad + weigth * rayRad + (1-galpha)*transmit * residualRayRad
            const float3 rayRad = weight * grad;
            radiance += rayRad;
            residualRayRad = maxf3((nextTransmit <= minTransmittance ? make_float3(0) : (integratedRadiance - radiance) / nextTransmit),
                                   make_float3(0));
        }
