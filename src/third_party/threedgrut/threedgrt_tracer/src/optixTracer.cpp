...
// SPDX-FileCopyrightText: Copyright (c) 2026 Kai Kohyama. All rights reserved.
...
OptixTracer::trace(uint32_t frameNumber,
    ...
    const torch::TensorOptions opts = torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA);
    TORCH_CHECK(rayOri.dim() == 3 || rayOri.dim() == 4,
                "rayOri must have shape (B, H, W, 3) or (B, N, 3), got ", rayOri.sizes());
    TORCH_CHECK(rayDir.dim() == rayOri.dim(), "rayDir must match rayOri rank");
    TORCH_CHECK(rayOri.size(-1) == 3, "last dimension of rayOri must be 3");
    TORCH_CHECK(rayDir.size(-1) == 3, "last dimension of rayDir must be 3");
    TORCH_CHECK(rayDir.sizes() == rayOri.sizes(), "rayDir must have the same shape as rayOri");

    const bool flattenedRays = (rayOri.dim() == 3);

    torch::Tensor rayOriAccessor = flattenedRays
                                       ? rayOri.view({rayOri.size(0), 1, rayOri.size(1), rayOri.size(2)})
                                       : rayOri;
    torch::Tensor rayDirAccessor = flattenedRays
                                       ? rayDir.view({rayDir.size(0), 1, rayDir.size(1), rayDir.size(2)})
                                       : rayDir;

    std::vector<int64_t> radianceShape = rayOri.sizes().vec();
    std::vector<int64_t> scalarShape   = radianceShape;
    scalarShape.back()                 = 1;
    std::vector<int64_t> hitShape      = radianceShape;
    hitShape.back()                    = 2;

    torch::Tensor rayRad       = torch::zeros(radianceShape, opts);
    torch::Tensor rayDns       = torch::empty(scalarShape, opts);
    torch::Tensor rayHit       = torch::empty(hitShape, opts);
    torch::Tensor rayNrm       = torch::zeros(radianceShape, opts);
    torch::Tensor rayHitsCount = torch::zeros(scalarShape, opts);
    torch::Tensor particleVisibility = torch::zeros({particleDensity.size(0), 1}, opts);

    torch::Tensor rayRadAccessorTensor = flattenedRays
                                             ? rayRad.view({rayRad.size(0), 1, rayRad.size(1), rayRad.size(2)})
                                             : rayRad;
    torch::Tensor rayDnsAccessorTensor = flattenedRays
                                             ? rayDns.view({rayDns.size(0), 1, rayDns.size(1), rayDns.size(2)})
                                             : rayDns;
    torch::Tensor rayHitAccessorTensor = flattenedRays
                                             ? rayHit.view({rayHit.size(0), 1, rayHit.size(1), rayHit.size(2)})
                                             : rayHit;
    torch::Tensor rayNrmAccessorTensor = flattenedRays
                                             ? rayNrm.view({rayNrm.size(0), 1, rayNrm.size(1), rayNrm.size(2)})
                                             : rayNrm;
    torch::Tensor rayHitsCountAccessorTensor = flattenedRays
                                                   ? rayHitsCount.view({rayHitsCount.size(0), 1, rayHitsCount.size(1),
                                                                        rayHitsCount.size(2)})
                                                   : rayHitsCount;
    ...
    paramsHost.frameBounds.x = rayOriAccessor.size(2) - 1;
    paramsHost.frameBounds.y = rayOriAccessor.size(1) - 1;
    ...
    paramsHost.rayOrigin    = packed_accessor32<float, 4>(rayOriAccessor);
    paramsHost.rayDirection = packed_accessor32<float, 4>(rayDirAccessor);

    const bool requestNormals   = (renderOpts & MOGRenderWithNormals) != 0;
    const bool requestHitCounts = (renderOpts & MOGRenderWithHitCounts) != 0;

    paramsHost.outputFlags = MOGOutputRadiance | MOGOutputDensity | MOGOutputHitDistance | MOGOutputVisibility;
    if (requestNormals) {
        paramsHost.outputFlags |= MOGOutputNormals;
    }
    if (requestHitCounts) {
        paramsHost.outputFlags |= MOGOutputHitCounts;
    }

    paramsHost.particleDensity      = getPtr<const ParticleDensity>(particleDensity);
    paramsHost.particleRadiance     = (paramsHost.outputFlags & MOGOutputRadiance) ? getPtr<const float>(particleRadiance) : nullptr;
    ...
    paramsHost.rayRadiance    = packed_accessor32<float, 4>(rayRadAccessorTensor);
    paramsHost.rayDensity     = packed_accessor32<float, 4>(rayDnsAccessorTensor);
    paramsHost.rayHitDistance = packed_accessor32<float, 4>(rayHitAccessorTensor);
    paramsHost.rayNormal      = packed_accessor32<float, 4>(rayNrmAccessorTensor);
    paramsHost.rayHitsCount   = packed_accessor32<float, 4>(rayHitsCountAccessorTensor);
    ...
    const unsigned int launchWidth  = static_cast<unsigned int>(rayRadAccessorTensor.size(2));
    const unsigned int launchHeight = static_cast<unsigned int>(rayRadAccessorTensor.size(1));
    const unsigned int launchDepth  = static_cast<unsigned int>(rayRadAccessorTensor.size(0));

    OPTIX_CHECK(optixLaunch(_state->pipelineTracingFwd, cudaStream, _state->paramsDevice,
                            sizeof(PipelineParameters), &_state->sbtTracingFwd, launchWidth,
                            launchHeight, launchDepth));
...
OptixTracer::traceBwd(uint32_t frameNumber,
    ...
    TORCH_CHECK(rayOri.dim() == 3 || rayOri.dim() == 4,
                "rayOri must have shape (B, H, W, 3) or (B, N, 3), got ", rayOri.sizes());
    TORCH_CHECK(rayDir.sizes() == rayOri.sizes(), "rayDir must have the same shape as rayOri");
    TORCH_CHECK(rayRad.sizes() == rayOri.sizes(), "rayRad must have the same shape as rayOri");
    TORCH_CHECK(rayNrm.sizes() == rayOri.sizes(), "rayNrm must have the same shape as rayOri");
    TORCH_CHECK(rayOri.size(-1) == 3, "last dimension of rayOri must be 3");
    TORCH_CHECK(rayDir.size(-1) == 3, "last dimension of rayDir must be 3");
    TORCH_CHECK(rayRad.size(-1) == 3, "last dimension of rayRad must be 3");
    TORCH_CHECK(rayNrm.size(-1) == 3, "last dimension of rayNrm must be 3");
    ...
    auto to4dView = [&](const torch::Tensor& tensor, int64_t expectedLastDim, const char* tensorName) {
        TORCH_CHECK(tensor.dim() == rayOri.dim(), tensorName, " must have the same rank as rayOri");
        TORCH_CHECK(tensor.size(0) == rayOri.size(0), tensorName, " batch dimension mismatch");
        if (flattenedRays) {
            TORCH_CHECK(tensor.size(1) == rayOri.size(1), tensorName, " flattened ray dimension mismatch");
        } else {
            TORCH_CHECK(tensor.size(1) == rayOri.size(1) && tensor.size(2) == rayOri.size(2),
                        tensorName, " spatial dimensions must match rayOri");
        }
        TORCH_CHECK(tensor.size(-1) == expectedLastDim,
                    tensorName, " last dimension must be ", expectedLastDim);

        return flattenedRays ? tensor.view({tensor.size(0), 1, tensor.size(1), tensor.size(2)}) : tensor;
    };

    torch::Tensor rayRadAccessor      = to4dView(rayRad, 3, "rayRad");
    torch::Tensor rayDnsAccessor      = to4dView(rayDns, 1, "rayDns");
    torch::Tensor rayHitAccessor      = to4dView(rayHit, 2, "rayHit");
    torch::Tensor rayNrmAccessor      = to4dView(rayNrm, 3, "rayNrm");
    torch::Tensor rayRadGradAccessor  = to4dView(rayRadGrd, 3, "rayRadGrd");
    torch::Tensor rayDnsGradAccessor  = to4dView(rayDnsGrd, 1, "rayDnsGrd");
    torch::Tensor rayHitGradAccessor  = to4dView(rayHitGrd, 2, "rayHitGrd");
    torch::Tensor rayNrmGradAccessor  = to4dView(rayNrmGrd, 3, "rayNrmGrd");
    ...
    paramsHost.gradFlags = MOGGradientParticleDensity | MOGGradientParticleRadiance;

    paramsHost.particleDensity      = getPtr<const ParticleDensity>(particleDensity);
    paramsHost.particleRadiance     = (paramsHost.outputFlags & MOGOutputRadiance) ? getPtr<const float>(particleRadiance) : nullptr;
    paramsHost.particleExtendedData = reinterpret_cast<const void*>(_state->gPipelineParticleData);

    paramsHost.rayRadiance    = packed_accessor32<float, 4>(rayRadAccessor);
    paramsHost.rayDensity     = packed_accessor32<float, 4>(rayDnsAccessor);
    paramsHost.rayHitDistance = packed_accessor32<float, 4>(rayHitAccessor);
    paramsHost.rayNormal      = packed_accessor32<float, 4>(rayNrmAccessor);

    paramsHost.particleDensityGrad  = getPtr<ParticleDensity>(particleDensityGrad);
    paramsHost.particleRadianceGrad = (paramsHost.gradFlags & MOGGradientParticleRadiance) ? getPtr<float>(particleRadianceGrad) : nullptr;

    paramsHost.rayRadianceGrad    = packed_accessor32<float, 4>(rayRadGradAccessor);
    paramsHost.rayDensityGrad     = packed_accessor32<float, 4>(rayDnsGradAccessor);
    paramsHost.rayHitDistanceGrad = packed_accessor32<float, 4>(rayHitGradAccessor);
    paramsHost.rayNormalGrad      = packed_accessor32<float, 4>(rayNrmGradAccessor);
    ...
    const unsigned int launchWidth  = static_cast<unsigned int>(rayRadAccessor.size(2));
    const unsigned int launchHeight = static_cast<unsigned int>(rayRadAccessor.size(1));
    const unsigned int launchDepth  = static_cast<unsigned int>(rayRadAccessor.size(0));
...
OptixTracer::traceDepth(uint32_t frameNumber,
    ...
    torch::Tensor rayRad       = torch::empty(radianceShape, opts);
    torch::Tensor rayDns       = torch::empty(scalarShape, opts);
    torch::Tensor rayHit       = torch::empty(hitShape, opts);
    torch::Tensor rayNrm       = torch::empty(radianceShape, opts);
    torch::Tensor rayHitsCount = torch::zeros(scalarShape, opts);
    torch::Tensor particleVisibility = torch::zeros({particleDensity.size(0), 1}, opts);
    ...
    paramsHost.outputFlags          = MOGOutputDensity | MOGOutputHitDistance | MOGOutputVisibility;

    paramsHost.particleDensity      = getPtr<const ParticleDensity>(particleDensity);
    paramsHost.particleRadiance     = nullptr;
    paramsHost.particleExtendedData = reinterpret_cast<const void*>(_state->gPipelineParticleData);
    paramsHost.particleVisibility   = getPtr<int32_t>(particleVisibility);
    ...
    return std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>(rayDns, rayHit, rayHitsCount, particleVisibility);
}

torch::Tensor
OptixTracer::traceDepthBwd(uint32_t frameNumber,
                      torch::Tensor rayToWorld,
                      torch::Tensor rayOri,
                      torch::Tensor rayDir,
                      torch::Tensor rayDns,
    ...
    paramsHost.outputFlags = MOGOutputDensity | MOGOutputHitDistance | MOGOutputVisibility;
    paramsHost.gradFlags   = MOGGradientParticleDensity;
    ...
    const unsigned int launchWidth  = static_cast<unsigned int>(rayHitAccessor.size(2));
    const unsigned int launchHeight = static_cast<unsigned int>(rayHitAccessor.size(1));
    const unsigned int launchDepth  = static_cast<unsigned int>(rayHitAccessor.size(0));

    OPTIX_CHECK(optixLaunch(_state->pipelineTracingBwd, cudaStream, _state->paramsDevice,
                            sizeof(PipelineBackwardParameters), &_state->sbtTracingBwd,
                            launchWidth, launchHeight, launchDepth));

    return torch::Tensor(particleDensityGrad);
}