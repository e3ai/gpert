...
extern "C" __global__ void __raygen__rg() {
    ...
    const bool computeRadiance        = (params.outputFlags & MOGOutputRadiance) != 0;
    const bool computeDensity         = (params.outputFlags & MOGOutputDensity) != 0;
    const bool computeHitDistance     = (params.outputFlags & MOGOutputHitDistance) != 0;
    const bool propagateRadianceGrad  = (params.gradFlags & MOGGradientParticleRadiance) != 0;
    const bool propagateDensityGrad   = (params.gradFlags & MOGGradientParticleDensity) != 0;

    const float3 rayOrigin    = params.rayWorldOrigin(idx);
    const float3 rayDirection = params.rayWorldDirection(idx);

    float3 rayIntegratedRadiance = make_float3(0.f);
    if (computeRadiance) {
        rayIntegratedRadiance = make_float3(params.rayRadiance[idx.z][idx.y][idx.x][0], params.rayRadiance[idx.z][idx.y][idx.x][1], params.rayRadiance[idx.z][idx.y][idx.x][2]);
    }
    float rayIntegratedTransmittance = computeDensity ? (1.0f - params.rayDensity[idx.z][idx.y][idx.x][0]) : 1.0f;
    float rayIntegratedHitDistance   = computeHitDistance ? params.rayHitDistance[idx.z][idx.y][idx.x][0] : 0.0f;
    float rayMaxHitDistance          = computeHitDistance ? params.rayHitDistance[idx.z][idx.y][idx.x][1] : RayHit::InfiniteDistance;

    float3 rayRadianceGrad = make_float3(0.f);
    if (propagateRadianceGrad) {
        rayRadianceGrad = make_float3(params.rayRadianceGrad[idx.z][idx.y][idx.x][0], params.rayRadianceGrad[idx.z][idx.y][idx.x][1], params.rayRadianceGrad[idx.z][idx.y][idx.x][2]);
    }
    float rayTransmittanceGrad = propagateDensityGrad ? (-1.0f * params.rayDensityGrad[idx.z][idx.y][idx.x][0]) : 0.0f;
    float rayHitDistanceGrad   = computeHitDistance ? params.rayHitDistanceGrad[idx.z][idx.y][idx.x][0] : 0.0f;
        ...
        for (int i = 0; i < PipelineParameters::MaxNumHitPerTrace; i++) {
            ...
            if (rayHit.particleId != RayHit::InvalidParticleId) {
                processHitBwd<PipelineParameters::ParticleKernelDegree, PipelineParameters::SurfelPrimitive>(
                    ...
                    computeRadiance ? params.particleRadiance : nullptr,
                    propagateRadianceGrad ? params.particleRadianceGrad : nullptr,
