...
class Tracer:
    ...
    class _AutogradDepth(torch.autograd.Function):
        @staticmethod
        def forward(
            ctx,
            tracer_wrapper,
    ...
    class _AutogradDepthDns(torch.autograd.Function):
        @staticmethod
        def forward(
            ctx,
            tracer_wrapper,
    ...
    def render_depth(self, gaussians, gpu_batch: Batch, train=False, frame_id=0):
        num_gaussians = gaussians.num_gaussians
        with torch.cuda.nvtx.range(f"model.forward({num_gaussians} gaussians)"):
            if self.frame_timer is not None:
                self.frame_timer.start()
    ...
    def render_depth_density(
        self, gaussians, gpu_batch: Batch, train=False, frame_id=0
    ):
        num_gaussians = gaussians.num_gaussians
        with torch.cuda.nvtx.range(f"model.forward({num_gaussians} gaussians)"):
            if self.frame_timer is not None:
            ...
            (
                ...
                hits_count,
                mog_visibility,
            ) = Tracer._AutogradDepthDns.apply(
                self.tracer_wrapper,
                frame_id,
                gpu_batch.T_to_world.contiguous(),
                gpu_batch.rays_ori.contiguous(),
