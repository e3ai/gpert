...
from ..model.model import MixtureOfGaussians
from ..strategy.base import BaseStrategy
from ..utils.logger import logger
from ..utils.misc import quaternion_to_so3, check_step_condition
...
class GSStrategy(BaseStrategy):
    ...
    def densify_gaussians(self, scene_extent):
        ...
        max_n_gaussians = self.model.max_n_gaussians
        if max_n_gaussians is not None and self.model.num_gaussians >= max_n_gaussians:
            return
    ...
    def prune_gaussians_scale(self, dataset):
        ...
        if isinstance(poses, torch.Tensor):
            poses_tensor = poses.to(
                device=self.model.device, dtype=self.model.positions.dtype
            )
        else:
            poses_tensor = torch.from_numpy(poses).to(
                device=self.model.device, dtype=self.model.positions.dtype
            )

        if poses_tensor.ndim == 2 and poses_tensor.shape[-1] == 7:
            positions = poses_tensor[:, :3]
            quats = poses_tensor[:, 3:]
            rotations = quaternion_to_so3(quats)
            poses_h = (
                torch.eye(4, dtype=poses_tensor.dtype, device=poses_tensor.device)
                .unsqueeze(0)
                .repeat(rotations.shape[0], 1, 1)
            )
            poses_h[:, :3, :3] = rotations
            poses_h[:, :3, 3] = positions
            poses_tensor = poses_h

        cam_normals = poses_tensor[:, :3, 2]
        similarities = torch.matmul(self.model.positions, cam_normals.T)
        cam_dists = similarities.min(dim=1)[0].clamp(min=1e-8)
        ratio = self.model.get_scale().min(dim=1)[0] / cam_dists * max(dataset.K)
