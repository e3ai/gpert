...
class Trainer3DGRUT:
    ...
    def on_training_end(self):
        ...
        if conf.export_ingp.enabled:
            ingp_path = (
                conf.export_ingp.path
                if conf.export_ingp.path
                else os.path.join(out_dir, "export_last.ingp")
            )
        ...
        if conf.export_usdz.enabled:
            usdz_path = (
                conf.export_usdz.path
                if conf.export_usdz.path
                else os.path.join(out_dir, "export_last.usdz")
            )
            exporter = USDZExporter()
            exporter.export(
                self.model, Path(usdz_path), dataset=self.train_dataset, conf=conf
            )
        ...
        for iter, batch in enumerate(self.train_dataloader):
            ...
            with torch.cuda.nvtx.range(f"train_{global_step}_backprop"):
                if isinstance(model.optimizer, SelectiveAdam):
                    assert outputs["mog_visibility"].shape == model.density.shape, (
