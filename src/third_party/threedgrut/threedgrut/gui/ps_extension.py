...
def set_custom_cugl_bindings():
    ...
    ps_device_func_dict = {
        "map_resource_and_get_array": lambda handle: _3dgrut_gui_plugin.cuda.map_graphics_resource_array(
            handle
        ),
        "unmap_resource": lambda handle: _3dgrut_gui_plugin.cuda.unmap_graphics_resource(
            handle
        ),
        "register_gl_buffer": lambda native_id: _3dgrut_gui_plugin.cuda.register_gl_buffer(
            native_id
        ),
        "register_gl_image_2d": lambda native_id: _3dgrut_gui_plugin.cuda.register_gl_texture(
            native_id
        ),
        "unregister_resource": lambda handle: _3dgrut_gui_plugin.cuda.unregister_cuda_resource(
            handle
        ),
        "get_array_ptr": lambda input_array: (input_array.data_ptr(), None, None, None),
        "memcpy_2d": lambda dst_ptr,
        src_ptr,
        width,
        height: _3dgrut_gui_plugin.cuda.memcpy_2d_to_array_async(
