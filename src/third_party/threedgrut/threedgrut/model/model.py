...
from .background import make
from ..datasets.protocols import Batch
from ..datasets.utils import read_next_bytes, read_colmap_points3D_text
from ..export.base import ExportableModel
from ..export.ingp_exporter import INGPExporter
from ..export.ply_exporter import PLYExporter
from ..model.geometry import nearest_neighbor_dist_cpuKD, k_nearest_neighbors
...
from ..utils.render import RGB2SH
...
class MixtureOfGaussians(torch.nn.Module, ExportableModel):
    ...
    def __init__(self, conf, scene_extent=None):
        ...
        self.max_n_features = (
            self.conf.model.progressive_training.max_n_features
        )  # For SH, this is the SH degree
        ...
        if conf.render.method == "3dgrt":
            self.renderer = Tracer(conf)
        else:
            raise ValueError(f"Unknown rendering method: {conf.render.method}")

        self.max_n_gaussians = getattr(self.conf.strategy, "max_n_gaussians", None)
    ...
    def default_initialize_from_points(
        ...
        if colors is None:
            colors = torch.randint(
                0, 256, (N, 3), dtype=torch.uint8, device=self.device
            )
    ...
    def forward(
        self, batch: Batch, train=False, frame_id=0, render_type="radiance", depth_grad=None
        ...
        Args:
            ...
            render_type: a string indicating the type of rendering to perform. ["radiance", "depth"]
            depth_grad: a string indicating the type of depth gradient to compute.
                "density": backpropagate loss to only the density parameters of gaussians
                "posrot": backpropagate loss to density, position and rotation parameters
            A dictionary containing the output of the model
        """
        if render_type == "radiance":
            return self.renderer.render(self, batch, train, frame_id)
        elif render_type == "depth":
            if depth_grad == "density":
                return self.renderer.render_depth_density(self, batch, train, frame_id)
            elif depth_grad == "posrot":
                return self.renderer.render_depth(self, batch, train, frame_id)
    ...
    def init_from_ingp(self, ingp_path, init_model=True):
        ...
        positions = (
            torch.from_numpy(
                np.frombuffer(mogt_config["mog_positions"], dtype=import_dtype)
            )
        ...
        scales = (
            torch.from_numpy(
                np.frombuffer(mogt_config["mog_scales"], dtype=import_dtype)
            )
            .to(device=self.device)
            .reshape(mog_num, 3)
        )
        densities = (
            torch.from_numpy(
                np.frombuffer(mogt_config["mog_densities"], dtype=import_dtype)
            )
            .to(device=self.device)
            .reshape(mog_num, 1)
        )
        rotations = (
            torch.from_numpy(
                np.frombuffer(mogt_config["mog_rotations"], dtype=import_dtype)
            )
            .to(device=self.device)
            .reshape(mog_num, 4)
        )
        ...
        features = (
            torch.from_numpy(
                np.frombuffer(mogt_config["mog_features"], dtype=import_dtype)
            )
            .to(device=self.device)
