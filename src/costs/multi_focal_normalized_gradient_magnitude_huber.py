...
from . import CostBase, NormalizedGradientMagnitudeHuber

logger = logging.getLogger(__name__)


class MultiFocalNormalizedGradientMagnitudeHuber(CostBase):
    """Multi-focus (reference) normalized gradient magnitude.
    ...
    name = "multi_focal_normalized_gradient_magnitude_huber"
    required_keys = [
        "forward_iwe",
        "backward_iwe",
        "middle_iwe",
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.gradient_loss = NormalizedGradientMagnitudeHuber(
    ...
    def calculate_torch(
        ...
        """Calculate gradient magnitude of IWE.
        Inputs:
        ...
        """
        forward_loss = self.gradient_loss.calculate_torch(
            forward_iwe, orig_iwe, omit_boundary
        )
        backward_loss = self.gradient_loss.calculate_torch(
            backward_iwe, orig_iwe, omit_boundary
        )
        loss = forward_loss + backward_loss

        if middle_iwe is not None:
            loss += (
                self.gradient_loss.calculate_torch(middle_iwe, orig_iwe, omit_boundary)
                * 2
            )
    ...
    # def calculate_numpy(self, events: np.ndarray, : np.ndarray, omit_boundary: bool) -> float:
    def calculate_numpy(
        self,
        orig_iwe: np.ndarray,
        ...
        """
        forward_loss = self.gradient_loss.calculate_numpy(
            forward_iwe, orig_iwe, omit_boundary
        )
        backward_loss = self.gradient_loss.calculate_numpy(
            backward_iwe, orig_iwe, omit_boundary
        )
        loss = forward_loss + backward_loss
        ...
        if middle_iwe is not None:
            loss += (
                self.gradient_loss.calculate_numpy(middle_iwe, orig_iwe, omit_boundary)
                * 2
            )
